...
(def cli-options
  [["-s" "--string" "Directly interpret the string passed as last argument, 
                     instead of considering it as a filepath"]
   ["-r" "--representation REPR" "Define what intermediate representation
                                  to interpret (one of \"e\", \"cfg\"),
                                  defaults to \"e\""
    :default "e"
    :validate [(fn [val] (some #{val} ["e" "cfg"])) "Representation must
                                                     be one of \"e\", \"cfg\""]]
   ["-h" "--help"]])
...
(defn -main [& args]
  (println
   (let [{errors :errors
          argmts :arguments
          opts :options} (parse-opts args cli-options)]
     (if errors
       (first errors)
       (if (< (count argmts) 1)
